...
public class GuiGolemBook extends GuiScreen {
	...
	private static final void initGolemBookEntries(World world) {
		...
		Collections.sort(GOLEMS, new Comparator<GolemBookEntry>() {
			@Override
			public int compare(GolemBookEntry g1, GolemBookEntry g2) {
				return Float.compare(g1.getAttack(), g2.getAttack());
			}
		});
		...
		Collections.sort(ALPHABETICAL, new Comparator<GolemBookEntry>() {
			@Override
			public int compare(GolemBookEntry g1, GolemBookEntry g2) {
				return g1.getGolemName().compareTo(g2.getGolemName());
			}
		});
	}
